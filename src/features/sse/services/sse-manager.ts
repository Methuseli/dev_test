...
export class SSEManager {
  ...
  broadcast(event: SSEEvent, options: SSEBroadcastOptions = {}): number {
    ...
    if (options.sessionId) {
      clients = clients.filter((client) => client.sessionId === options.sessionId);
    }
  ...
  getStats() {
    ...
    return {
      ...
      userConnections: new Set(clients.map((c) => c.userId).filter(Boolean)).size,
      sessionConnections: new Set(clients.map((c) => c.sessionId).filter(Boolean)).size,
